...
class AddStockConsumeView < ActiveRecord::Migration
  def up
    execute "CREATE OR REPLACE VIEW v_stock_consume_in AS " +
      " SELECT a.t_location_id,a.equipment_id,a.brand,b.model,sum(a.qty) qty,sum(a.qty*price) amt FROM inout_lines a,equipment b " +
    ...
    execute "CREATE OR REPLACE VIEW v_stock_consume_out AS " +
      " SELECT a.f_location_id,a.equipment_id,a.brand,b.model,sum(a.qty) qty,sum(a.qty*price) amt FROM inout_lines a,equipment b " +
      " WHERE a.state='confirmed' AND a.equipment_id = b.id" +
      " AND b.type = 'EquipConsume'" +
