...
class CreateStockInView < ActiveRecord::Migration
  def change
    execute "CREATE OR REPLACE VIEW v_stock_in AS " +
      "SELECT t_location_id,equipment_id,sum(qty) qty,sum(qty*price) amt FROM inout_lines group by t_location_id,equipment_id WHERE state='confirmed'"
    execute "CREATE OR REPLACE VIEW v_stock_out AS " +
      "SELECT f_location_id,equipment_id,sum(qty) qty,sum(qty*price) amt FROM inout_lines group by f_location_id,equipment_id WHERE state='confirmed'"
