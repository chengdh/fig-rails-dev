...
class Match < ActiveRecord::Base
  ...
  #sb滚球数据
  scope :sb_list, -> {joins(:current_match).includes(:league,:team1,:team2,:odds_asians,:odds_europes,:odds_balls).where("t_current_match.match_status not in (-1,-10)").order("t_match.match_time ASC")}
