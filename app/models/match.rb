...
class Match < ActiveRecord::Base
  ...
  scope :immediate,-> {joins(:current_match).includes(:league,:team1,:team2,:match_recommands).where("(t_current_match.match_status > 0) OR (match_time >= ? and match_time <= ?)",
                                                                                                     DateTime.now.hour + 8  <= 12 ? DateTime.now.end_of_day + 8.hours - 36.hours : DateTime.now.end_of_day + 8.hours  - 12.hours ,
                                                                                                     DateTime.now.hour + 8 <= 12 ? DateTime.now.end_of_day  + 8.hours- 12.hours : DateTime.now.end_of_day + 8.hours + 12.hours ).order("t_current_match.match_status DESC,t_match.match_time ASC")
  }
  #
